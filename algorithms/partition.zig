    ...
    less: fn(a: T, b: T) bool) Pair()
{
    ...
    while( (end - begin) > 1 and less(input_data[begin], pivot1) and less(pivot2, input_data[end - 1]) ) {
        begin += 1;
        end -= 1;
    }

    // first skip elements already correctly in the first partiton
    while( begin < end and less(input_data[begin], pivot1) )
    {
        begin += 1;
    }
...
pub inline fn compareAndSwap(comptime T: type, a: *T, b: *T, less: fn(a: T, b: T) bool) void
{
    // assume(&a != &b);
    // auto a_t = Move(a);
    // auto b_t = Move(b);
...
fn sort3(comptime T: type, a: *T, b: *T, c: *T, less: fn(a: T, b: T) bool) void
{
    compareAndSwap(T, a, b, less);
    compareAndSwap(T, b, c, less);
    compareAndSwap(T, a, b, less);
}


fn median3(comptime T: type, a_in: *T, b_in: *T, c_in: *T, comptime less: fn(a: T, b: T) bool) *T
{
    var a = a_in;
    var b = b_in;
    var c = c_in;
    const pointer = struct {
        pub fn less(a2: *T, b2: *T) bool {
            return less(a2.*, b2.*);
        }
    };
    sort3(*T, &a, &b, &c, pointer.less);
    return b;
}


pub fn partition3ByEstimatedMedian(
    comptime T: type,
    data: []T,
    comptime less: fn(a: T, b: T) bool) Pair() {
    var pivot = median3(T, &data[0], &data[data.len/2], &data[data.len - 1], less);
    ...
    comptime less: fn(a: T, b: T) bool) void
{
    ...
    while(partition_data.len > 16) {
        var current_partition_elements = partition3ByEstimatedMedian(T, partition_data, less);
    ...
    std.sort.insertionSort(T, partition_data, lessThan);
}
...
test "DualPivotPartition with fixed values." {
    ...
    var values = [85]i64{-119, -34, -69, -56, 2, -111, -145, -128, -83, -153, -154, -3,
        -158, -126, -153, -109, -151, -101, -22, -132, -53, -97, -28, 12, -102, -95, -27,
        -26, -139, -30, -40, 2, -155, -105, -119, -62, -52, -146, -28, 7, -107, -160, -112,
        -134, -30, -161, -4, -127, -113, -32, 6, -13, -117, -31, -130, -102, -38, -148, 1,
        -155, -164, -152, -4, -151, -130, -34, -86, -64, -66, -20, -80, -48, -162, 13, -128,
        -78, -22, -15, -47, 2, -137, -107, -21, -80, -158 };
